...
FreeRTOS/             FreeRTOS kernel 原始碼與 Posix port(不要修改)
project/              正式專案
...
demo/                 各種獨立 demo
  ...
  semaphore_demo.c    範例 demo,實作 demo_start()
Makefile
README.md             英文(主檔,以此為準)
...
- `main()` 只負責開機流程；實際邏輯放在 `app_start()`(正式專案)或
  `demo_start()`(demo)。
- `hook.c` 提供應用層必須實作的回呼(目前是 `configASSERT` 失敗時呼叫的
  `vAssertCalled`),兩種建置都會連結。
...
| `make run` | — | 建置並執行正式專案 |
| `make demo` | `build/demo_semaphore` | 建置預設 demo(semaphore) |
| `make run-demo` | — | 建置並執行預設 demo |
| `make demo DEMO=foo` | `build/demo_foo` | 建置 `demo/foo_demo.c` |
| `make run-demo DEMO=foo` | — | 建置並執行指定 demo |
| `make clean` | — | 刪除 `build/` |

`DEMO` 變數選擇要建哪一個 demo 模組,預設為 `semaphore`。

> 註:demo 是無窮迴圈程式,用 `Ctrl-C` 結束;若用 `timeout 3 ./build/demo_semaphore`
> 觀察輸出,離開碼 `124` 代表是 `timeout` 正常中止,並非錯誤。

## 新增或修改程式的步驟

### A. 開發正式專案

1. 在 [project/app.c](project/app.c) 的 `app_start()` 裡建立你的 tasks、
   queues、semaphores 等,成功回傳 `pdPASS`、失敗回傳 `pdFAIL`。
...
> 慣例:每個 demo 的對外函式一律命名為 `demo_start`。因為一次只連結一個 demo,
> 所以不會撞名;demo 內部的其他函式請用 `static`。
...
kernel 預設值。若要啟用新功能,記得同時補上對應設定與來源檔,例如:

- **software timers**:把 `configUSE_TIMERS` 設為 `1`,並將
  `FreeRTOS/timers.c` 加進 Makefile 的 `COMMON_SOURCES`。
- **各種 hook**(idle/tick/malloc-failed…):把對應的 `configUSE_*_HOOK` 設為
  `1`,並在 [project/hook.c](project/hook.c) 補上該回呼函式的實作,否則連結時會
  出現 `undefined reference`。

> `projCOVERAGE_TEST` 與 `projENABLE_TRACING` 由 Makefile 以 `-D` 在命令列定義,
> 不要寫死在設定檔裡。
